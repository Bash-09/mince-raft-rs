...
pub mod packets;
use miniz_oxide::{deflate::{compress_to_vec, compress_to_vec_zlib}, inflate::{decompress_to_vec, decompress_to_vec_zlib}};
use packets::*;
                ...
                Some(packet) => {
                    match packet {
                        ...
                        DecodedPacket::SetCompression(threshold) => {
                            if threshold.0 <= 0 {
                                self.compress = false;
                                self.channel.send.send(NetworkCommand::Log(Log::new(LogType::Info("Disabled Compression".to_string())))).expect("Failed to send log to main thread.");
                            } else {
                                self.compress = true;
                                self.threshold = threshold.0 as usize;
                                self.channel.send.send(NetworkCommand::Log(Log::new(LogType::Info(format!("Set Compression: {}", threshold.0).to_string())))).expect("Failed to send log to main thread.");
                            }
                        ...
                        _ => {println!("Got unexpected packet during login: {:?}", packet);}
                    }
                },
                None => {},
            }
            ...
            Some(pack) => {
                ...
                if self.compress {
                    ...
                    if bytes.len() >= self.threshold {
                        ...
                        let packet_length = VarInt((compressed.len()+data_length.num_bytes()) as i32);
    ...
    fn handle_packet(&mut self, packet: DecodedPacket) {
        ...
        match &packet {
            ...
            KeepAliveClientbound(keep_alive_id) => {
                self.send_packet(KeepAliveServerbound(keep_alive_id.clone())).expect("Failed to send heartbeat");
            },
