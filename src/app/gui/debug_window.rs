...
impl DebugWindow {
    ...
    pub fn render(&mut self, ui: &Ui, server: &Server) {
        ...
        .build(&ui, || {
            ...
            if server.difficulty_locked {difficulty_locked = "(Locked)";} 
            else {difficulty_locked = "";}
            ui.text(im_str!("Difficulty: {:?} {}", server.difficulty, difficulty_locked));

            let period: &str;
            let day_time = server.day_time%24000;
            if day_time < 6000 {         period = "Sunrise";
            } else if day_time < 12000 { period = "Noon";
            } else if day_time < 18000 { period = "Sunset";
            } else {                     period = "Midnight"}
            ui.text(im_str!("Day: {}    Time: {} ({})", server.world_time/24000, day_time, period));
            ...
            ui.text(im_str!("Pitch: {}", server.player.orientation.get_head_pitch()));
